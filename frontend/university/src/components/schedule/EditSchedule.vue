...
export default {
  ...
  methods: {
    getSchedule(id){
      ScheduleS.getById(id).then((response)=>{
    ...
    updateSchedule(){
      ScheduleS.update(this.schedule.id, this.schedule).then(response=>{console.log('Updated successfully')})
      this.$router.go(-1)
    }
  },
  created(){
    this.getSchedule(this.$route.params.sche_id)
    this.getDisciplines()
    this.getTeacher()
  }
}
